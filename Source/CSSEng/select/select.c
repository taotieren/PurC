    ...
    void* clone_node, void* _node_data)
{
    ...
    if (handler == NULL || node_data == NULL || handler->handler_version != CSS_SELECT_HANDLER_VERSION_1) {
        return CSS_BADPARM;
    }
    ...
    css_computed_style** style)
{
    ...
    if (ctx == NULL || style == NULL || handler == NULL || handler->handler_version != CSS_SELECT_HANDLER_VERSION_1)
        return CSS_BADPARM;
    ...
    struct css_node_data** sharable_node_data)
{
    ...
    *sharable_node_data = NULL;
    ...
    if ((node_data->flags & CSS_NODE_FLAGS_HAS_HINTS) != (state->node_data->flags & CSS_NODE_FLAGS_HAS_HINTS)) {
#ifdef DEBUG_STYLE_SHARING
     ...
     * can't share. */
    if ((node_data->flags & CSS_NODE_FLAGS__PSEUDO_CLASSES_MASK) != (state->node_data->flags & CSS_NODE_FLAGS__PSEUDO_CLASSES_MASK)) {
#ifdef DEBUG_STYLE_SHARING
    ...
    if (node_data->flags & (CSS_NODE_FLAGS_TAINT_PSEUDO_CLASS | CSS_NODE_FLAGS_TAINT_ATTRIBUTE | CSS_NODE_FLAGS_TAINT_SIBLING | CSS_NODE_FLAGS_HAS_INLINE_STYLE)) {
#ifdef DEBUG_STYLE_SHARING
        printf("      \t%s\tno share: candidate flags: %s%s%s%s\n",
            ...
            (node_data->flags & CSS_NODE_FLAGS_HAS_INLINE_STYLE) ? " INLINE_STYLE" : "");
#endif
        return CSS_OK;
    }
    ...
    void* pw)
{
    ...
    state->pw = pw;
    state->next_reject = state->reject_cache + (N_ELEMENTS(state->reject_cache) - 1);
    ...
    css_select_results** result)
{
    ...
    if (ctx == NULL || node == NULL || result == NULL || handler == NULL || handler->handler_version != CSS_SELECT_HANDLER_VERSION_1)
        return CSS_BADPARM;
    ...
    if (nhints > 0) {
        ...
        struct css_computed_style* computed_style = state.results->styles[CSS_PSEUDO_ELEMENT_NONE];
    ...
    for (i = 0; i < ctx->n_sheets; i++) {
        ...
        if (mq__list_match(s.media, media) && s.sheet->disabled == false) {
    ...
    if (inline_style != NULL) {
        ...
        /* Sanity check style */
        if (inline_style->rule_count != 1 || inline_style->rule_list->type != CSS_RULE_SELECTOR || inline_style->rule_list->items != 0) {
            error = CSS_INVALID;
            goto cleanup;
        }
    ...
    for (i = 0; i < CSS_N_PROPERTIES; i++) {
        const prop_state* prop = &state.props[i][CSS_PSEUDO_ELEMENT_NONE];

        /* If the property is still unset or it's set to inherit
         * and we're the root element, then set it to its initial
         * value. */
        if (prop->set == false || (parent == NULL && prop->inherit == true)) {
    ...
    css_select_font_faces_results** result)
{
    ...
    n_font_faces = state.ua_font_faces.count + state.user_font_faces.count + state.author_font_faces.count;
    ...
    if (n_font_faces > 0) {
        ...
        if (state.ua_font_faces.count != 0) {
            memcpy(results->font_faces,
                ...
                sizeof(css_font_face*) * state.ua_font_faces.count);
        ...
        if (state.user_font_faces.count != 0) {
            memcpy(results->font_faces + i,
                ...
                sizeof(css_font_face*) * state.user_font_faces.count);
        ...
        if (state.author_font_faces.count != 0) {
            memcpy(results->font_faces + i,
                ...
                sizeof(css_font_face*) * state.author_font_faces.count);
        }
    ...
    void* parent)
{
     ...
     */
    if (prop_dispatch[prop].inherited == false || (pseudo == CSS_PSEUDO_ELEMENT_NONE && parent == NULL)) {
    ...
    css_origin origin, css_select_state* state, size_t* nr_matched_selectors)
{
    ...
    do {
        ...
        if (rule != NULL && rule->type == CSS_RULE_IMPORT) {
            ...
            if (import->sheet != NULL && mq__list_match(import->media, state->media)) {
    ...
    css_combinator comb, const css_selector* s)
{
    ...
    if (state->next_reject < state->reject_cache || comb != CSS_COMBINATOR_ANCESTOR || next_detail == NULL || next_detail->next != 0 || (next_detail->type != CSS_SELECTOR_CLASS && next_detail->type != CSS_SELECTOR_ID))
        return;
    ...
    bool* match_chain)
{
    ...
    *match_chain = false;
    ...
    do {
        ...
        if (s->data.comb != CSS_COMBINATOR_NONE && s->combinator->data.qname.name != ctx->universal) {
            /* Named combinator */
            may_optimise &= (s->data.comb == CSS_COMBINATOR_ANCESTOR || s->data.comb == CSS_COMBINATOR_PARENT);
            ...
            /* No match for combinator, so reject selector chain */
            if (next_node == NULL) {
                if (may_optimise && s == selector && rejected_by_cache == false) {
    ...
    do {
        ...
        case CSS_COMBINATOR_SIBLING:
            ...
            if (error != CSS_OK)
                return error;
            if (node == state->node) {
                state->node_data->flags |= CSS_NODE_FLAGS_TAINT_SIBLING;
            }
            break;
        ...
        if (n != NULL) {
             ...
             * give up. */
            if (type == CSS_COMBINATOR_PARENT || type == CSS_COMBINATOR_SIBLING)
                n = NULL;
        }
    } while (n != NULL);
    ...
    void** next_node)
{
    ...
    *rejected_by_cache = false;
    ...
    if (may_optimise && (type == CSS_COMBINATOR_ANCESTOR || type == CSS_COMBINATOR_PARENT) && next_detail != NULL && (next_detail->type == CSS_SELECTOR_CLASS || next_detail->type == CSS_SELECTOR_ID)) {
        reject_item* reject = state->next_reject + 1;
        reject_item* last = state->reject_cache + N_ELEMENTS(state->reject_cache) - 1;
        bool match = false;

        while (reject <= last) {
            /* Perform pessimistic matching (may hurt quirks) */
            if (reject->type == next_detail->type && lwc_string_isequal(reject->value, next_detail->qname.name, &match) == lwc_error_ok && match) {
    ...
    bool* match, css_pseudo_element* pseudo_element)
{
    ...
    case CSS_SELECTOR_PSEUDO_CLASS:
        ...
        if (is_root == false && detail->qname.name == ctx->first_child) {
            ...
            if (error == CSS_OK)
                *match = (num_before == 0);
        } else if (is_root == false && detail->qname.name == ctx->nth_child) {
            int32_t num_before = 0;
        ...
        } else if (is_root == false && detail->qname.name == ctx->nth_last_child) {
            int32_t num_after = 0;
        ...
        } else if (is_root == false && detail->qname.name == ctx->nth_of_type) {
            int32_t num_before = 0;
        ...
        } else if (is_root == false && detail->qname.name == ctx->nth_last_of_type) {
            int32_t num_after = 0;
        ...
        } else if (is_root == false && detail->qname.name == ctx->last_child) {
            int32_t num_after = 0;
            ...
            if (error == CSS_OK)
                *match = (num_after == 0);
        } else if (is_root == false && detail->qname.name == ctx->first_of_type) {
            int32_t num_before = 0;
            ...
            if (error == CSS_OK)
                *match = (num_before == 0);
        } else if (is_root == false && detail->qname.name == ctx->last_of_type) {
            int32_t num_after = 0;
            ...
            if (error == CSS_OK)
                *match = (num_after == 0);
        } else if (is_root == false && detail->qname.name == ctx->only_child) {
            ...
            if (error == CSS_OK) {
                error = state->handler->node_count_siblings(
                    ...
                    &num_after);
                if (error == CSS_OK)
                    *match = (num_before == 0) && (num_after == 0);
            }
        } else if (is_root == false && detail->qname.name == ctx->only_of_type) {
            int32_t num_before = 0, num_after = 0;
    ...
    bool inherit)
{
    ...
    if (existing->set == 0) {
        ...
        if (existing->origin < state->current_origin) {
             ...
             * one is USER, i. */
            if (existing->important == 0 || existing->origin != CSS_ORIGIN_USER) {
                outranks = true;
            }
             ...
             * one is USER, i. */
            if (state->current_origin == CSS_ORIGIN_USER && important) {
                outranks = true;
            }
        }
    }
...
void dump_chain(const css_selector* selector)
{
    ...
    do {
        switch (detail->type) {
        case CSS_SELECTOR_ELEMENT:
            if (lwc_string_length(detail->qname.name) == 1 && lwc_string_data(detail->qname.name)[0] == '*' && detail->next == 1) {
                break;
            }
            fprintf(stderr, "%.*s",
    ...
    void* node, css_select_handler* handler, void* pw, bool* match)
{
    ...
    if (!selector || !node || !handler || !pw || !match || handler->handler_version != CSS_SELECT_HANDLER_VERSION_1) {
        err = CSS_BADPARM;
        goto out;
    }
